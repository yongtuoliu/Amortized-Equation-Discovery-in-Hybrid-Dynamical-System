...
def _learning_rate_warmup(global_step, warmup_end_lr, warmup_start_lr, warmup_steps):
...
def get_learning_rate(global_step, config):
    ...
    if config["flat_learning_rate"]: # False
        lr_schedule = config["learning_rate"] # 0.0004
    else:
        if global_step < config["warmup_steps_lr"]: # 2000
            lr_schedule = _learning_rate_warmup(
                global_step,
                config["learning_rate"], # 0.0004
                config["warmup_start_lr"], # 5.e-5
                config["warmup_steps_lr"], # 2000
            )
        else:
            decay_steps = config["num_steps"] - config["warmup_steps_lr"] # 80000-2000
            decay_steps = min(config.get("lr_decay_steps", decay_steps), decay_steps)
...
def _schedule_exponential_decay(
    global_step, decay_steps, kickin_steps, init_val, annealing_rate, min_val=1e-10
):
    ...
    temp_schedule = (
        init_val if global_step < kickin_steps else max(decay_schedule, min_val)
    )
...
def get_cross_entropy_coef(step, config):
    ...
    if config["xent_annealing"]: # True
        cross_entropy_schedule = _schedule_exponential_decay(
            step,
            decay_steps=config["xent_steps"], # 50
            kickin_steps=config["xent_kickin_steps"], # 2000
            init_val=config["xent_init"], # 100
            annealing_rate=config["xent_rate"], # 0.99
        )
    else:
...
def get_config_and_setup_dirs(filename="config.yaml"):
    ...
    config["model_best_dir"] = config["model_best_dir"].format(timestamp=timestamp)
    os.makedirs(config["log_dir"])
    os.makedirs(config["model_dir"])
    os.makedirs(config["model_best_dir"])
