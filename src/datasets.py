...
class SpringMassDataset(torch.utils.data.Dataset):
    def __init__(self, path="./data/10012023Hopping_dataset.mat", data_type="train", n_obj=1):
        with open(path, "rb") as tf:
            data_dict = sio.loadmat(tf)

        str_inputs = 'y_out_' + data_type
        str_inputs_d = 'a_out_' + data_type
        str_seg = 's_out_' + data_type
       
        data_inputs = np.expand_dims(np.transpose(np.array(data_dict[str_inputs], dtype=np.float32), (2,0,1)), axis=1) # num_samples, num_objects, num_timesteps, num_variables (2000,1,150,2)
        data_inputs_max = np.max(data_inputs, axis=(0,1,2))
        data_inputs_min = np.min(data_inputs, axis=(0,1,2))
        data_inputs_normalized = (data_inputs - data_inputs_min) / (data_inputs_max - data_inputs_min)

        data_inputs_d = np.expand_dims(np.transpose(np.array(data_dict[str_inputs_d], dtype=np.float32), (2,0,1)), axis=1) # num_samples, num_objects, num_timesteps, num_variables (2000,1,150,2)
        data_inputs_max_d = np.max(data_inputs_d, axis=(0,1,2))
        data_inputs_min_d = np.min(data_inputs_d, axis=(0,1,2))
        data_inputs_normalized_d = (data_inputs_d - data_inputs_min_d) / (data_inputs_max_d - data_inputs_min_d)

        data_seg = np.expand_dims(np.transpose(np.array(data_dict[str_seg], dtype=np.float32), (2,0,1)), axis=1) # num_samples, num_objects, num_timesteps, num_variables (2000,1,150,1)

        self.data_aggre = np.concatenate((data_inputs_normalized, data_inputs_normalized_d, data_seg), axis=3) # (2000,1,150,5)

    def __getitem__(self, i):
        return self.data_aggre[i]

    def __len__(self):
        return len(self.data_aggre)
...
class GTSUnivariateDataset(torch.utils.data.IterableDataset):
    ...
    def __init__(
        ...
        if mode == "train":
            ...
            pre_split_tfs = Chain(
                input_transform.transformations[:-1] + [SelectFields(fields_to_keep)]
            )
            splitter = input_transform.transformations[-1]
            assert isinstance(splitter, CanonicalInstanceSplitter)

            transformed_dataset = TransformedDataset(dataset.train, pre_split_tfs)
