...
class DiscreteStateTransition(nn.Module):
    ...
    def forward(self, y, x, hidden_states, ctrl_feats=None):
        """The forward pass.
        ...
        """
        B, T = y.shape[:2]
        inputs_to_net_eachobject = []
        if self.takes_x:
            inputs_to_net_eachobject += [x]
        if self.takes_y:
            inputs_to_net_eachobject += [y]
        if self.takes_hidden_states:
            inputs_to_net_eachobject += [hidden_states]
        if self.takes_ctrl:
            assert (
                ctrl_feats is not None
            ), "ctrl_feats cannot be None when self.takes_ctrl = True!"
            inputs_to_net_eachobject += [ctrl_feats]
        inputs_to_net_eachobject_final = torch.cat(inputs_to_net_eachobject, dim=-1)

        if self.with_interaction:
            # Modeling Interactions
            self.n_batch = int(B / self.n_samples / self.n_obj)
            # print(y.shape, x.shape, hidden_states.shape)
            x_expand = x.view(self.n_samples, self.n_batch, self.n_obj, T, self.x_size)
            y_expand = y.view(self.n_samples, self.n_batch, self.n_obj, T, self.y_size)
            hidden_states_expand = hidden_states.view(self.n_samples, self.n_batch, self.n_obj, T, self.hidden_states_size)
            # y_sign_expand = y_sign.view(self.n_samples, self.n_batch, self.n_obj)
            # print(torch.where(y_sign_expand==0))
            # a, b, c = torch.where(y_sign_expand==0)
            x_all_objects = x_expand.permute(0,1,3,2,4).reshape(self.n_samples, self.n_batch, T, self.n_obj*self.x_size)
            y_all_objects = y_expand.permute(0,1,3,2,4).reshape(self.n_samples, self.n_batch, T, self.n_obj*self.y_size)
            hidden_states_all_objects = hidden_states_expand.permute(0,1,3,2,4).reshape(self.n_samples, self.n_batch, T, self.n_obj*self.hidden_states_size)
            # print(x_all_objects.shape, y_all_objects.shape, hidden_states_all_objects.shape)
            ...
            if self.takes_ctrl:
                ...
                inputs_to_net_holistic += [ctrl_feats]
            inputs_to_net_holistic = torch.cat(inputs_to_net_holistic, dim=-1)
            inputs_to_net_holistic_expand = inputs_to_net_holistic[:,:,None,:,:].repeat(1,1,self.n_obj,1,1)
            inputs_to_net_holistic_final = inputs_to_net_holistic_expand.view(self.n_samples*self.n_batch*self.n_obj, T, -1)
            inputs_to_net_final = torch.cat((inputs_to_net_holistic_final, inputs_to_net_eachobject_final), -1)
        else:
            inputs_to_net_final = inputs_to_net_eachobject_final

        if len(inputs_to_net_final) == 0:
            # No recurrence
            dummy_inputs = torch.ones(B, T, 1, device=y.device)
            inputs_to_net_eachobject = []
            inputs_to_net_eachobject += [dummy_inputs]
            inputs_to_net_final = torch.cat(inputs_to_net_eachobject, dim=-1)

        transition_tensor = self.network(inputs_to_net_final).view([B, T, self.K, self.K])
...
class RNNInferenceNetwork(nn.Module):
    ...
    def forward(self, y, ctrl_feats=None, num_samples=1, deterministic=False):
        ...
        x_samples = x_samples.permute(1, 0, 2).view(num_samples, B, T, latent_dim)
        ...
        hidden_states_list= hidden_states_list.permute(1, 0, 2).view(num_samples, B, T, self.posterior_rnn.hidden_size)
        return x_samples, entropies, log_probs, hidden_states_list