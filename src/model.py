...
class Base(nn.Module):
    def __init__(
        ...
        if discrete_state_prior is None:
            self.register_buffer("discrete_prior", torch.ones([self.K]) / self.K)
        else:
    ...
    def _calculate_continuous_transition_probabilities(
        ...
        log_prob_xt = p_xt_dists.log_prob(future_xts[:, :, None, :]) # 新增None为不同category
    ...
    def _calculate_discrete_transition_probabilities(
        ...
        log_prob_zt_ztm1 = torch.cat([log_identity_pad, log_prob_zt_ztm1], dim=1)
    ...
    def _calculate_likelihoods(
        ...
        log_prob_x0 = x0_dist.log_prob(x0_samples[:, None, :]) # 新增None为不同category
        log_prob_x0 = log_prob_x0[:, None, :] # 新增None为不同timestep
        ...
        log_p_xt_yt = log_prob_yt[:, :, None] + log_prob_xt # 新增None为不同category
        return log_p_xt_yt
...
class SNLDS(Base):
    ...
    def _calculate_loglike_lowerbound(
        ...
        t2 = torch.sum(
            ...
            dim=[1, 2, 3],
        ) # There maybe some errors here log_b[:, 1:, :, None]
    ...
    def _get_iwlbo(
        ...
        loglike_p_XY_lowerbound = loglike_p_XY_lowerbound.view([num_samples, -1])
        log_surrogate_posterior = log_surrogate_posterior.view([num_samples, -1])

        iwlbo = torch.logsumexp(
            loglike_p_XY_lowerbound - log_surrogate_posterior, dim=0
        ) - np.log(num_samples)
        # iwlbo = iwlbo.mean(0)
        iwlbo = torch.sum(iwlbo*y_sign) / torch.sum(y_sign)
        return iwlbo
    ...
    def _calculate_objective(
        ...
        entropy_term = torch.sum(torch.sum(x_entropy, dim=1)*y_sign) / torch.sum(y_sign)
        #  likelihood_termv1 is the lower-bound on p(X, Y) proposed by
        #  Dong et. al, 2020.
        # likelihood_termv1 = loglike_p_XY_lowerbound.mean(0)
        likelihood_termv1 = torch.sum(loglike_p_XY_lowerbound*y_sign) / torch.sum(y_sign)
        #  likelihood_termv2 is p(X, Y) computed using the forward algorithm.
        # likelihood_termv2 = log_p_XY.mean(0)
        likelihood_termv2 = torch.sum(log_p_XY*y_sign) / torch.sum(y_sign)
        elbo = likelihood_termv1 + entropy_term
        elbov2 = likelihood_termv2 + entropy_term
        iwlbo = self._get_iwlbo(y_sign, loglike_p_XY_lowerbound, log_prob_q, num_samples)
        iwlbov2 = self._get_iwlbo(y_sign, log_p_XY, log_prob_q, num_samples)
    ...
    def forward(
        ...
        if self.transform_target:
            with torch.no_grad():
                if self.transform_only_scale:
                    scale_y = torch.mean(torch.abs(y), dim=-2, keepdim=True) + eps
                    ...
                    target_transformer = td.transforms.AffineTransform(mu_y, std_y)
        ...
        if self.ctrl_transformer is not None:
            ...
            ctrl_feats = self.ctrl_transformer(
                ...
                feat_time=ctrl_inputs["past_time_feat"][..., : self.context_length, :],
            )
        ...
        x_samples, x_entropy, log_prob_q, hidden_states = self.inference_network(
            y,
        ...
        hidden_states = hidden_states.view(num_samples * B, T, -1)

        #  Repeat the first dim num_samples times to allow broadcast with x_samples.
        y_tiled = y.repeat(num_samples, 1, 1)
        y_sign_tiled = y_sign.repeat(num_samples)
        ...
        #  Compute log A, A = p(z[t] | x[t-1], z[t-1]).
        log_a = self._calculate_discrete_transition_probabilities(
            y_tiled, x_samples, hidden_states, ctrl_feats_tiled, temperature=switch_temperature
        )
        #  Compute gamma, xi, and, log p(X, Y) using the forward-backward algorithm.
        ...
        # )
        crossent_regularizer = (
            torch.sum(torch.einsum("ijk, k -> ij", log_gamma, self.discrete_prior).sum(1)*y_sign_tiled) / torch.sum(y_sign_tiled)
        )
    ...
    def _unroll(
        ...
        for i in range(T):
            ...
            if deterministic_x:
                ...
                xtp1 = xtp1_dist.sample()[torch.arange(0, num_samples), 0, ztp1, :]
            zt = ztp1
            xt = xtp1
            z_samples.append(zt)
    ...
    def predict(
        ...
        rec_y = self.get_reconstruction(x_samples).view([num_samples, B, T, -1])
...
class AMORE(Base):
    ...
    def _get_iwlbo(
        self, loglike_p_XY: torch.Tensor, log_prob_q: torch.Tensor, num_samples: int
    ):
        log_surrogate_posterior = torch.sum(log_prob_q, dim=-1)
        ...
        iwlbo = torch.logsumexp(loglike_p_XY - log_surrogate_posterior, dim=0) - np.log(
            num_samples
        )
        iwlbo = iwlbo.mean(0)
        # iwlbo = torch.sum(iwlbo*y_sign) / torch.sum(y_sign)
    ...
    def forward(
        ...
        #   Compute the KL between the discrete prior and gamma.
        crossent_regularizer = (
            torch.einsum("ijk, k -> ij", log_z_posterior, self.discrete_prior).sum(1).mean(0)
        )
        # crossent_regularizer = (
        #     torch.sum(torch.einsum("ijk, k -> ij", log_z_posterior, self.discrete_prior).sum(1)*y_sign_tiled) / torch.sum(y_sign_tiled)
        # )
    ...
    def _unroll(
        ...
        for i in range(T):
            ...
            u = log_u.exp()[torch.arange(0, num_samples), 0, zt.squeeze(), ct.squeeze()]
            ...
            ctp1 = ct
            if not (
                u_sample.size() == ctp1.size() and torch.unique(u_sample).size(0) <= 2
            ):
                print(u_sample.size(), ctp1.size(), torch2numpy(torch.unique(u_sample)))
                raise RuntimeError()
    ...
    def predict(
        ...
        if ctrl_feats_tiled is None:
            #  If the dataset is control-less,
            #  then feed in dummy control = 1
            dummy_ctrls = torch.ones(num_samples * B, T, 1, device=log_a.device)
            log_u = self.ctrl2nstf_network(ctrl_feats=dummy_ctrls)
        else:
            log_u = self.ctrl2nstf_network(ctrl_feats=ctrl_feats_tiled)
        #  Compute gamma using the forward-backward
        #  algorithm.
        _, _, log_gamma, _ = forward_backward_hsmm(log_a, log_b, log_z_init, log_u)
        #  Reconstruct the input
        rec_y = self.get_reconstruction(x_samples).view([num_samples, B, T, -1])
        #  Get the most likely state for cT, zT
        log_gamma_flat = log_gamma.view(num_samples * B, T, self.K * self.d_max)
        zTcT = unravel_indices(
            torch.argmax(log_gamma_flat[:, -1, :], dim=-1), (self.K, self.d_max)
        )
