...
def build_model(config):
    ...
    z_init = InitialDiscreteDistribution(
        network=z0_network,
        num_categories=config["num_categories"],
        takes_ctrl=config["control"]["z"], # False
    )
    ...
    z_transition = DiscreteStateTransition(
        ...
        takes_y=config["discrete_transition"]["takes_y"], # False
        takes_hidden_states=config["discrete_transition"]["takes_hidden_states"], # False
        with_interaction=config["discrete_transition"]["with_interaction"], # False
        n_samples=config["num_samples"], # 1
        n_obj=config["n_obj"], # 1
        x_size=config["x_dim"], # 8
        y_size=config["obs_dim"], # 2
        hidden_states_size=config["inference"]["causal_rnndim"], # 16
    )
    ...
    if config["model"] == "SNLDS":
        ...
        model = AMORE(
            ...
            transform_target=config["transform_target"], # False
            transform_only_scale=config["transform_only_scale"], # False
            use_jacobian=config["use_jacobian"], # False
        )
    return model
    ...
    if (
        ...
        if config["control"]["has_ctrl"] and config["control"]["z"]: # False False
            z0_network = nn.Sequential(
                ...
                nn.ReLU(True),
                nn.Linear(
                    config["initial_switch"]["mlp_hiddendim"], config["num_categories"]
                ),
            )
        else:
            z0_network = nn.Linear(1, config["num_categories"], bias=False)

        # Discrete Transition
        if config["discrete_transition"]["with_interaction"]:
            n_interaction_objs = config["n_obj"]
        else: 
            n_interaction_objs = 0

        discrete_in_dim = (
            config["x_dim"]*(n_interaction_objs+1) if config["discrete_transition"]["takes_x"] else 0
        )
        discrete_in_dim += (
            config["obs_dim"]*(n_interaction_objs+1) if config["discrete_transition"]["takes_y"] else 0
        )
        discrete_in_dim += (
            config["inference"]["causal_rnndim"]*(n_interaction_objs+1) if config["discrete_transition"]["takes_hidden_states"] else 0
        )
        ...
        if discrete_in_dim + z_ctrl_dim > 0:
            z_transition_network = nn.Sequential(
                nn.Linear(
                    discrete_in_dim + z_ctrl_dim, 4 * (config["num_categories"] ** 2)
                ),
                nn.ReLU(True),
                nn.Linear(
                    4 * (config["num_categories"] ** 2), config["num_categories"] ** 2
                ),
            )
        else:
